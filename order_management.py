...
class OrderWithState:
    def __init__(self, id: str, order: str, state: int, order_date: datetime, estimated_arrival_date: datetime):
        self.id = id
        self.order = order
        self.state = state
...
class OrderManagement:
    ...
    def add_order(self, order: str) -> None:
        ...
        with shelve.open(self.db_file) as db:
            ...
            estimated_arrival_date: datetime = datetime.now() + timedelta(days=10) # Set the estimated arrival date to be 10 days from now
    ...
    def get_all_orders(self) -> List[OrderWithState]:
        with shelve.open(self.db_file) as db:
            if self.user_identifier in db:
                return db[self.user_identifier]  # Return the list of orders for the user
